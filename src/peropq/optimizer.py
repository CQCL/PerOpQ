...
class Optimizer:
    def __init__(self, hamiltonian: Hamiltonian, R: int = 3, t: float = 1.0):
        ...
        self.uvar = Uvar(n_term=self.n_terms, R=self.R, cjs=self.cjs, t=self.t)
        self.uvar.set_theta_to_Trotter()
        self.cache = {}

    def C2_squared(self, thetas: Sequence[float] | None = None):
        if theta != None:
            theta_new = np.array(thetas).reshape((self.uvar.R - 1, self.uvar.n_terms))
            self.uvar.update_theta(theta_new)
        if self.cache == {}:
            commutator_list = []
            indices_list = []
            for j, H_j in enumerate(self.commuting_pauli_string_list):
                for j_prime, H_j_prime in enumerate(self.commuting_pauli_string_list):
                    if j > j_prime:
            ...
            for k, k_commutator in enumerate(self.cache["commutator_list"]):
                if j != k:
                    ...
                    if product_commutators != 0:
                        c2_squared_result -= (
                            self.uvar.chi(self.cache["indices_list"][j][0][1])
                            * self.uvar.chi(self.cache["indices_list"][k][0][1])
                            * product_commutator.normalized_trace()
                        )
        return c2_squared_result

    def get_minumum_c2_squared(self, initial_guess: Sequence[float] = None):
        if initial_guess != None:
            x0 = initial_guess.reshape((self.uvar.R - 1, self.uvar.n_terms))
        else:
            x0 = self.uvar.get_initial_trotter_vector()
        optimized_results = scipy.optimize.minimize(self.C2_squared, x0)
        return optimized_results, self.C2_squared(thetas=optimized_results.x)
